...
class SawyerReachYZEnvV3(SawyerXYZEnv):
    """Reach to a target in the YZ plane (X is fixed to current EE position).

    The goal is sampled in the YZ plane at the same X as the end effector's
    ...
    def __init__(
        ...
        # Goal varies in Y and Z; X is fixed at hand_init_pos X
        goal_low = np.array([self.hand_init_pos[0], 0.4, 0.02])
        goal_high = np.array([self.hand_init_pos[0], 0.8, 0.4])
    ...
    def reset_model(self) -> npt.NDArray[np.float64]:
        # Randomize hand init position in YZ, keep X fixed
        ...
        self.hand_init_pos[0] = self.init_config["hand_init_pos"][0]
        ...
        # Force X of target to match hand init X
        self._target_pos[0] = self.hand_init_pos[0]
